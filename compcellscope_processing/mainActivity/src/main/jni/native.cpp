...
JNIEXPORT void JNICALL Java_com_choochootrain_refocusing_tasks_ComputeDPCRefocusTask_computeDPCRefocus(JNIEnv* env, jobject, jfloat zMin, jfloat zStep,
    ...
    __android_log_write(ANDROID_LOG_INFO, "DPC", "loading images");
    int16_t imgCount = loadImages(datasetRoot,imageStack);
    __android_log_print(ANDROID_LOG_INFO, "DPC", "done loading images");
    Mat results[3];
    __android_log_print(ANDROID_LOG_INFO, "DPC", "created empty results matrix");

    for (float zDist = zMin; zDist <= zMax; zDist += zStep)
    {
        __android_log_print(ANDROID_LOG_INFO, "DPC", "Computing image for z=%f", zDist);
        computeFocusDPC(*imageStack, imgCount, zDist, imageStack->at(0).Image.cols, imageStack->at(0).Image.rows, 0, 0, results);
        __android_log_print(ANDROID_LOG_INFO, "DPC", "Done computing image for z=%f", zDist);
       
        char bfFilename[FILENAME_LENGTH];
        char dpcLRFilename[FILENAME_LENGTH];
        char dpcTBFilename[FILENAME_LENGTH];
        snprintf(bfFilename,sizeof(bfFilename), "/sdcard/Pictures/Refocused/BF_%3f.bmp",zDist);
        snprintf(dpcLRFilename,sizeof(dpcLRFilename), "/sdcard/Pictures/Refocused/DPCLR_%.2f.bmp",zDist);
        snprintf(dpcTBFilename,sizeof(dpcTBFilename), "/sdcard/Pictures/Refocused/DPCTB_%.2f.bmp",zDist);
       
        __android_log_print(ANDROID_LOG_INFO, "DPC", "writing images to MEEERP");
        imwrite(bfFilename, results[0]);
        imwrite(dpcLRFilename, results[1]);
        imwrite(dpcTBFilename, results[2]);
        __android_log_print(ANDROID_LOG_INFO, "DPC", "wrote images to file");
    }
}
...
int loadImages(string datasetRoot, vector<R_image> *images) {
    ...
    __android_log_write(ANDROID_LOG_INFO, "DPC", "entered loadImages function");
    DIR *dir;
    struct dirent *ent;
    if ((dir = opendir (datasetRoot.c_str())) != NULL) {
      ...
      while ((ent = readdir (dir)) != NULL) {
        __android_log_print(ANDROID_LOG_INFO, "DPC", "loaded image %d", num_images  );
        //add to list
        ...
        if (fileName.compare(".") != 0 && fileName.compare("..") != 0)
        {
            ...
            //currentImage.Image = imread(datasetRoot + "/" + fileName, CV_8UC3);
            //currentImage.Image = imread(datasetRoot + "/" + fileName, CV_LOAD_IMAGE_COLOR);
      ...
      closedir (dir);
      __android_log_print(ANDROID_LOG_INFO, "DPC", "about to return");
      return num_images;
